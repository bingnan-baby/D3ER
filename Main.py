...
class Coach:
   ...
   def run_boost(self):
      ...
      for i in self.model_name_list:
         self.model_ensemble.model_dict[i].load_state_dict(self.model_warmup.model_dict[i].state_dict())
   ...
   def trainEpoch_pretrain(self):
      ...
      epLoss, epClLoss, epSPLoss, epSWLoss = 0, 0, 0, 0
      ...
      for i, tem in enumerate(self.trnLoader):
         ...
         sw loss
         '''
         swLoss = args.sw_reg * sliced_wasserstein_distance(item_embed_visual_sh[poss], item_embed_text_sh[poss],num_projections=args.n_projection)
         ...
         log('Step %d/%d: cl : %.3f ; dis_v : %.3f; dis_t : %.3f; sp : %.3f ; gsw : %.3f \n' % (
            i,
            steps,
            clLoss.item(),
            (dis_visual),
   ...
   def trainEpoch_boost(self, stage, trained_model, optimizer):
      ...
      self.trnLoader = self.handler.trnLoader
      self.trnLoader.dataset.negSampling()
      epLoss, epBprLoss, epRegLoss = 0, 0, 0
      steps = self.trnLoader.dataset.__len__() // args.batch
      ...
      for i, tem in enumerate(self.trnLoader):
         ...
         if stage == 0:
            bprLoss = - (scoreDiff_cur).sigmoid().log().sum() / args.batch
         else:
            ...
            bprLoss = self.criterion(scoreDiff_cur, args.step_size * (1 - scoreDiff_ensem.sigmoid())) #
   ...
   def trainEpoch_distill(self, model_tmp, trained_model, trained_model_name, opt, stage):
      ...
      for i, tem in enumerate(self.trnLoader):
         ...
         if stage >= 3:
            ...
            distill_loss = self.criterion(scoreDiff_student, weight * scoreDiff_train) * args.distill_reg
         '''
         BPR Loss
         '''
         ...
         bprLoss = -(scoreDiff_ensemble).sigmoid().log().sum() / args.batch
         '''
         Reg Loss
         '''
         ...
         loss = distill_loss + bprLoss + regLoss
         loss.backward()
         opt.step()

         log(
            'Step %d/%d: distill loss : %.3f ; bpr loss : %.3f ; reg loss : %.3f ' % (
               i,
               steps,
               distill_loss.item(),
               bprLoss.item(),
